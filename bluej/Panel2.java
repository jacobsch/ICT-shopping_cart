...
import java.nio.file.Files; // Retrieve from external file
import java.nio.file.Paths; // Retrieve from path
...
public class Panel2 extends JPanel {
    ...
    private JButton btnLogin; // Adding the container locally so it does not interfere with the other panel files
    private JButton toRegister; // Adding the container locally so it does not interfere with the other panel files
    ...
    public void initComponents() {
        ...
        btnLogin.addActionListener( new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                String fileName = txtFirst.getText() + txtLast.getText() + ".json"; // recreating the json title so we can match the file with the user assuming they already created an account
                ...
                // If any of these boxs are empty a prompt will appear alerting user to fill in all required fields
                    ...
                    pwdPin.getText().equals(empty))
                {
                    ...
                    try {
                        ...
                        // if account name, email and password are a match this statement is true and it is executed
                      if (checkJSON(fileName, inputEmail, inputPin)) {
                        ...
                        JOptionPane.showMessageDialog(container, "Invalid!"); // Alert prompt saying info is Invalid!
                    }
    ...
    public boolean checkJSON(String fileName, String inputEmail, String inputPin) throws Exception {
        ...
        JSONObject databaseInfo = (JSONObject) readJSON(fileName); // init .json object of our file, basically bringing the file into our code

        String dbPin = String.valueOf(databaseInfo.get("pin")); // Creating the String to pull data out of json array
        String dbEmail = String.valueOf(databaseInfo.get("email")); // Creating the String to pull data out of json array
    ...
    public Object readJSON(String fileName) throws Exception {
        ...
        FileReader reader = new FileReader("accounts/" + fileName); // pulling file
